...
object ClassUtil {
    ...
    fun getClass(classLoader: ClassLoader, className: String, initialize: Boolean): Class<*>? {
        ...
        fun tryLoad(name: String): Class<*> {
            return PrimitiveUtil.getAbbreviation(name)?.let {
                forName("[$it").componentType
    ...
    private fun toCanonicalName(className: String): String {
        ...
        return prefix + (PrimitiveUtil.getAbbreviation(name) ?: "L$name;")
    }
    ...
    private fun isAssignable(clazz: Class<*>?, toClass: Class<*>?): Boolean {
        ...
        if (from.isPrimitive && !toClass.isPrimitive) {
            from = PrimitiveUtil.toWrapperClass(from) ?: return false
        }
        if (toClass.isPrimitive && !from.isPrimitive) {
            from = PrimitiveUtil.toPrimitiveClass(from) ?: return false
        }

        if (from == toClass) return true
        if (from.isPrimitive) {
            if (!toClass.isPrimitive) return false
            return PrimitiveUtil.canWiden(from, toClass)
        }
    ...
    private fun getPrimitivePromotionCost(srcClass: Class<*>, destClass: Class<*>): Float {
        ...
        val current = if (!srcClass.isPrimitive) {
            cost += 0.1f
            PrimitiveUtil.toPrimitiveClass(srcClass) ?: return 0f
        } else {
            srcClass
        }
