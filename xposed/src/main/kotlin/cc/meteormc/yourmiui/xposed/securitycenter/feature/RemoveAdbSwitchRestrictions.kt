...
object RemoveAdbSwitchRestrictions : XposedFeature(
    ...
    override fun onLoadPackage() {
        ...
        // 效果不太好而且毫无可读性 无奈之举
        operator("com.miui.permcenter.install.AdbInstallVerifyActivity") {
            ...
            val onCreateMethod = method("onCreate") ?: return@operator
            onCreateMethod.hookDoNothing {
                val activity = it.thisObject
                // 由于hook的是父级方法 所以判断当前环境是否为AdbInstallVerifyActivity
                if (!delegate.isInstance(activity)) return@hookDoNothing false
                ...
                operator(taskField.type()) suboperate@{
                    ...
                    val task = constructor(this@operator.delegate)?.new(activity) ?: return@suboperate false
