...
class HomeFragment : BaseFragment<FragmentHomeBinding>({ inflater, container ->
    ...
    private fun bindModuleStatus() {
        val isActivated = Bridge.isModuleActivated()
        ...
        val apiName = Bridge.getApiName()
        val apiVersion = Bridge.getApiVersion()
        val statusApiText = if (isActivated) "Activated${apiName?.let { " by $it (API ${apiVersion})" } ?: ""}" else "Not activated"
        ...
        binding.statusApi.text = statusApiText
    }
